...
- OG image & sharing
  ...
  - [x] Wire `og:image` meta tags on share pages to point to the generated OG endpoint. — Implemented server-side meta tags via `getServerSideProps` for `/compare/[slug]`.
  - [ ] view actual results in cli or browser

- DiceBear avatars (current priority)
  ...
  - Note: headshot uploads were moved to `documentation/sometimes.md` for later consideration (kept out of active next actions).
...
- Uploads, storage & Vercel notes
  - [ ] (moved) Headshot upload UX & storage design is documented in `documentation/sometimes.md` — not part of the immediate next actions.
  - [ ] Implement server-side support and documentation for headshot uploads (recommendation: Supabase Storage or S3) if you decide to enable uploads later.
